...
def main():
    ...
    # Main app content
    tab1, tab2, tab3 = st.tabs(["Upload Data", "Create Template", "Preview Messages"])
    ...
    if template:
        ...
        if placeholders:
            ...
            if st.session_state.contacts_df is not None:
                missing_fields = [p for p in placeholders if p not in st.session_state.contacts_df.columns]
                if missing_fields:
                    st.warning(f"The following placeholders are not in your CSV: {', '.join(missing_fields)}. You'll need to provide custom values for these.")
        else:
            st.warning("No placeholders found. Use {placeholder} syntax to create customizable messages.")
...
def preview_messages():
    ...
    missing_fields = [p for p in st.session_state.placeholders if p not in st.session_state.contacts_df.columns]
    
    if missing_fields:
        st.subheader("Custom Values for Missing Fields")
        st.write("These fields weren't found in your CSV. Please provide values for them:")
    ...
    if st.button("Generate Preview Messages"):
        ...
        has_remaining_placeholders = any('{' in msg and '}' in msg for msg in messages)
        
        if has_remaining_placeholders:
            st.error("Some placeholders couldn't be replaced. Please check your template and data.")
